 ...
 */
#ifndef __ARDUINO_AD5760
#define __ARDUINO_AD5760
...
class AD5760 {
  public:
    AD5760(uint8_t cs_pin);
    void setValue(uint32_t value);
    void enableOutput();
	void disableOutput();
	void readRegister(uint32_t value);
  ...
  protected:
    ...
    // The AD5760 needs just 16 bits of data so it needs to be shifted to left left by 4 bits
    static const uint32_t DAC_REGSISTER_VALUE_OFFSET = 4;

    // DAC value register (p. 21)
    static const uint32_t DAC_REGISTER = 0b001 << 20;
	static const uint32_t NOP_CONDITION = 0b000 << 20;
