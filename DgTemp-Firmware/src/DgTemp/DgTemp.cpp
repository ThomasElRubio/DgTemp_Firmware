...
void DgTemp::initFlexTimer(){
	...
	FTM0_C2SC |= 1<<5;        //MSB:MSA set to 1X (X means the state of this bit is not important)
	FTM0_C2SC &= ~(1<<3);		//ELSB
	FTM0_C2SC |= 1<<2;		//ELSA 
	FTM0_C2V = 192;			
	FTM0_C3SC |= 1<<5;        //MSB:MSA set to 1X (X means the state of this bit is not important)
	FTM0_C3SC &= ~(1<<2);
	FTM0_C3SC |= 1<<3;
	FTM0_C3V = 192;
